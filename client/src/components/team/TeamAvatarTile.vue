<template>
  ...
  >
    <button
      type="button"
      class="w-full h-full min-h-0 flex flex-col items-center justify-center gap-2 p-3 rounded-xl border border-app-border-subtle bg-app-surface shadow-card active:bg-app-bg min-w-0 overflow-visible"
    ...
    >
      ...
      >
        <span
          class="text-sm font-medium text-app-title text-center w-full min-w-0 line-clamp-2 leading-snug wrap-break-word"
        >
          {{ member.name }}
        </span>
      </span>
    ...
    <Teleport to="body">
      ...
      <Transition name="dropdown">
        <div
          ...
          ref="menuRef"
          class="fixed z-50 min-w-[140px] py-1 bg-app-surface rounded-lg shadow-lg border border-app-border-subtle"
          :style="{
        ...
        >
          <button
            type="button"
            class="w-full px-4 py-2.5 text-left text-sm text-app-title hover:bg-app-bg flex items-center gap-2"
            @click.stop="onEdit"
          >
          ...
          <button
            type="button"
            class="w-full px-4 py-2.5 text-left text-sm text-red-600 hover:bg-red-50 dark:hover:bg-red-500/20 flex items-center gap-2"
            @click.stop="onDelete"
          >
            <span aria-hidden>🗑️</span>
            Eliminar
...
<script setup lang="ts">
  ...
  const isManager = computed(() => props.member.role === "admin");
