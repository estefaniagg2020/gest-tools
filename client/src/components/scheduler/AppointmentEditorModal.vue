<template>
  ...
  >
    ...
    >
      ...
      >
        <span>✕</span>
        <span>{{ t('scheduler.appointmentCancelledBanner') }}</span>
      </div>

      <div
        v-else-if="isNoShow"
        class="flex items-center gap-2 px-3 py-2 rounded-lg bg-orange-50 border border-orange-200 text-orange-700 text-sm font-medium"
      >
        <span>👤✕</span>
        <span>{{ t('scheduler.noShowBanner') }}</span>
      </div>

      <div
        v-else-if="isCompleted"
        class="flex items-center gap-2 px-3 py-2 rounded-lg bg-emerald-50 border border-emerald-200 text-emerald-700 text-sm font-medium"
      >
        <span>✓</span>
        <span>{{ t('scheduler.completedBanner') }}</span>
      </div>
      ...
      >
        <span>⏰</span>
        <span>{{ t('scheduler.appointmentPastBanner') }}</span>
      </div>

      <div
        v-if="hasNoShowAlert"
        class="flex items-center gap-2 px-3 py-2 rounded-lg bg-red-50 border border-red-300 text-red-800 text-sm font-medium"
      >
        <span>⚠️</span>
        <span>{{ t('scheduler.noShowAlert', { count: consecutiveNoShows }) }}</span>
      </div>

      <div
        v-if="hasUnpaidAlert"
        class="flex items-center gap-2 px-3 py-2 rounded-lg bg-amber-50 border border-amber-300 text-amber-800 text-sm font-medium"
      >
        <span>💰</span>
        <span>{{ t('scheduler.unpaidAlert', { count: unpaidCount }) }}</span>
      </div>

      <div>
        <label class="block text-sm font-bold text-gray-700 mb-1">{{ t('scheduler.waitlistAddClient') }}</label>
        <ClientNameInput
          v-model:client-id="form.clientId"
          v-model:client-name="form.clientName"
          :options="clientOptions"
          :placeholder="t('scheduler.searchClient')"
          :empty-option-label="t('scheduler.waitlistSelectClient')"
          :disabled="isCancelled"
        />
      </div>

      <div>
        <label class="block text-sm font-bold text-gray-700 mb-1">{{ t('scheduler.waitlistAddService') }}</label>
        <SearchableSelect
          v-model="form.serviceId"
          :options="serviceOptions"
          :placeholder="t('scheduler.searchService')"
          :empty-option-label="t('scheduler.waitlistSelectService')"
          :disabled="isCancelled"
        />
      </div>

      <div>
        <label class="block text-sm font-bold text-gray-700 mb-1">{{ t('team.employee') }}</label>
        <SearchableSelect
          v-model="form.memberId"
          :options="memberOptions"
          :placeholder="t('scheduler.searchEmployee')"
          :empty-option-label="t('scheduler.selectEmployee')"
          :disabled="isCancelled"
        />
      </div>

      <div>
        <label class="block text-sm font-bold text-gray-700 mb-1">{{ t('scheduler.appointmentDate') }}</label>
        <input
      ...
      >
        <p class="text-sm font-semibold text-red-700">{{ t('scheduler.cancelAppointment') }}</p>
        <div>
          <label class="block text-xs font-bold text-gray-600 mb-1">{{ t('scheduler.cancelReasonLabel') }}</label>
          <textarea
            v-model="cancelReason"
            rows="2"
            :placeholder="t('scheduler.cancelReasonPlaceholder')"
        ...
        <div class="flex gap-2 justify-end">
          ...
          >
            {{ t('scheduler.confirmCancellation') }}
          </button>
        </div>
      </div>
      ...
      <div class="pt-5 border-t border-gray-100">
        ...
        <template v-if="editingId && isCancelled">
          <div class="flex items-center justify-end gap-3">
            ...
            >
              {{ t('scheduler.restoreAppointment') }}
            </BaseButton>
          </div>
        </template>

        <!-- Cita no-show -->
        <template v-else-if="editingId && isNoShow">
          <div class="flex items-center justify-end gap-3">
            <BaseButton
              variant="danger"
              class="rounded-xl! py-2.5!"
        ...
        <!-- Cita completada -->
        <template v-else-if="editingId && isCompleted">
          <div class="flex items-center justify-end gap-3">
            <button
              v-if="!isPaid"
              type="button"
...
<script setup lang="ts">
  ...
  import { useClientAppointmentAlerts } from "@/composables/useClientAppointmentAlerts";
  ...
  const clientIdRef = computed(() => form.clientId?.trim() ?? "");
  const { hasNoShowAlert, consecutiveNoShows, hasUnpaidAlert, unpaidCount } =
    useClientAppointmentAlerts(clientIdRef);
    ...
    if (memberId) {
      ...
      if (occupied) {
        const proceed = await showConfirm({
          title: t("scheduler.slotOccupiedTitle"),
          message: t("scheduler.slotOccupiedConfirm"),
          confirmLabel: t("scheduler.saveAnyway"),
          variant: "primary",
        });
        if (!proceed) return;
      }
...
<style scoped>
  ...
  .btn-appointment-warn {
    @apply px-4 py-2.5 rounded-xl text-sm font-semibold text-white transition-all duration-200;
    @apply bg-orange-500 hover:bg-orange-600 active:scale-[0.98];
    @apply shadow-md shadow-orange-500/20;
  }
</style>
