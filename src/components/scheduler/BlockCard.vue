<template>
  <div
    ...
    :class="[
      ...
      isAppointmentItem && isAppointmentCancelled ? 'opacity-60' : '',
      ...
      >
        ...
        <div class="flex items-center justify-between w-full mt-0.5 flex-wrap gap-x-1.5">
          ...
          <span class="flex items-center gap-1">
            ...
            <span class="text-xs px-1 rounded font-medium" :class="isAppointmentCancelled ? 'bg-red-100 text-red-600' : 'bg-white/60'">{{ appointmentStatusLabel }}</span>
          </span>
        </div>
      </div>
...
<script setup lang="ts">
  ...
  const appointmentCardStyle = computed(() => {
    ...
    const bgColor = apt.status === "cancelled"
      ? "var(--apt-cancelled-bg)"
      : apt.isVIP
        ? "var(--apt-vip-bg)"
        : apt.isAtHome
          ? "var(--apt-athome-bg)"
    ...
    return {
      ...
      color: apt.status === "cancelled" ? "var(--apt-cancelled-text)" : "var(--apt-normal-text)",
    };
  });
