...
export const useThemeStore = defineStore("theme", () => {
  ...
  // Apply theme/color-mode to document whenever config is loaded or changed
  watch(
    [themeId, colorMode, customColors, titleTextOverrides],
    ([id, mode, colors, overrides]) => {
      applyThemeToDocument(id as string, (colors as ThemeColors) ?? DEFAULT_CUSTOM_THEME_COLORS);
      applyColorModeToDocument((mode as ColorMode) ?? "system");
      applyTitleTextOverrides((overrides as { title: string; text: string } | null) ?? null);
    },
    { immediate: true },
  );
